...
- (IBAction)loginButtonTouched:(id)sender
{
    ...
    else
    {
        ...
        [[FIRAuth auth] signInWithEmail:_emailTextfield.text password:_passwordTextfield.text completion:^(FIRAuthDataResult * _Nullable authResult, NSError * _Nullable error)
        {
            if (error!=nil) {
                dispatch_async(dispatch_get_main_queue(), ^{
                    SCLAlertView *alert = [[SCLAlertView alloc] init];
                     [alert showError:self title:@"Login In" subTitle:error.localizedDescription closeButtonTitle:@"OK" duration:0.0f];
                });
...
-(void)getUserData
{
    ...
    [[[_ref child:@"users"] child:[[NSUserDefaults standardUserDefaults] objectForKey:@"uid"]] observeSingleEventOfType:FIRDataEventTypeValue withBlock:^(FIRDataSnapshot * _Nonnull snapshot) {
        // Get user value
           [[NSUserDefaults standardUserDefaults] setObject:[snapshot.value objectForKey:@"username"] forKey:@"email"];
         [[NSUserDefaults standardUserDefaults] setObject:[snapshot.value objectForKey:@"name"] forKey:@"name"];
...
*/
- (IBAction)guestLoginButtonTouched:(id)sender
{
    [[NSUserDefaults standardUserDefaults] setObject:@"Guest@guest.com" forKey:@"email"];
    [[NSUserDefaults standardUserDefaults] setObject:@"Guest" forKey:@"name"];
    UIStoryboard *sb = [UIStoryboard storyboardWithName:@"Main" bundle:nil];
    UIViewController *mainVC = [sb instantiateViewControllerWithIdentifier:@"HomeNav"];
    UIViewController *leftVC = [sb instantiateViewControllerWithIdentifier:@"LeftMenuViewController"];
